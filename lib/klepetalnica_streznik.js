...
function obdelajPosredovanjeSporocila(socket) {
  socket.on('sporocilo', function (sporocilo) {
    if (sporocilo.kanal) {
      /*if((sporocilo.charAt(0)=='<') && (sporocilo.charAt(1)=='i') && (sporocilo.charAt(0)=='m')) {
        console.log("slika");
  ...
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      ...
      if (socket.id==socketIdNaslovnika) {
       ...
       //io.sockets.sockets[socketIdNaslovnika].emit('sporocilo', { besedio: "Dregljaj od: " + dregljaj.vzdevek });
        io.sockets.sockets[socket.id].emit('sporocilo', {
      ...
      io.sockets.sockets[socket.id].emit('sporocilo', { besedilo: "Neznan ukaz." });  
    }
  });
}
